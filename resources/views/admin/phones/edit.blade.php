                    ...
                    <div class="col-sm-5">
                        <div class="form-group">
                            ...
                            <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text"
                                   name="name" id="name" value="{{ old('name', $phone->name) }}" required
                                   autofocus/>
                            @if($errors->has('name'))
                    ...
                    <div class="col-sm-2">
                        <div class="form-group">
                            ...
                            <select class="form-control select2-free {{ $errors->has('type') ? 'is-invalid' : '' }}"
                                    name="type" id="type">
                                @if (!$type_list->contains(old('type', $phone->type ?? '')))
                                    <option>{{ old('type', $phone->type ?? '') }}</option>
                                @endif
                                @foreach($type_list as $type)
                                    <option {{ old('type', $phone->type) == $type ? 'selected' : '' }}>{{$type}}</option>
                                @endforeach
                            </select>
                            @if($errors->has('type'))
                                <div class="invalid-feedback">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2-free-tags {{ $errors->has('attributes') ? 'is-invalid' : '' }}"
                                ...
                                @foreach($attributes_list as $a)
                                    <option {{ in_array($a, old('attributes', array_filter(explode(' ', (string) $phone->attributes)))) ? 'selected' : '' }}>{{ $a }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="col">
                        ...
                        <div class="form-group">
                            ...
                            <textarea
                                    ...
                                    name="description"
                                    id="description">{!! old('description', $phone->description) !!}</textarea>
                ...
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('address_ip') ? 'is-invalid' : '' }}" type="text"
                               name="address_ip" id="address_ip" value="{{ old('address_ip', $phone->address_ip) }}">
