...
The **Exploration** tool (`Tools → Exploration`) allows you to dynamically navigate through the asset relationship graph. It reveals dependencies between layers: upward (abstract / business) or downward (concrete / physical).
...
For highly connected assets (e.g., a central application), starting with 1 or 2 levels avoids an unreadable graph. Increase progressively from there.
...
**Physical Mode**
Enable the Physical toggle only when you want to visualize physical network links (WAN/LAN/MAN). When disabled, the exploration stays at the logical level.
...
- `certificates` (570) are cross-cutting: associate them to both applications AND logical servers for complete exploration.
- `external-connected-entities` (540) can be linked to the business layer (partners) or the network layer (connections) depending on context.
