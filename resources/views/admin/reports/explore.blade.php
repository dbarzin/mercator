...
<script>
    ...
    let _nodes = new Map();
    @foreach($nodes as $node)
        _nodes.set( "{{ $node["id"] }}" ,{ id: "{{ $node["id"]}}", vue: "{{ $node["vue"]}}", label: "{{ addslashes($node["label"]) }}", {{ array_key_exists('title',$node) ? ('title: "' . addslashes($node["title"]) . '",') : "" }} image: "{{ $node["image"] }}",  type: "{{ $node["type"] }}", edges: [ <?php
