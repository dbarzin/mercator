# Explorer and Dependencies

## 🌐 Two Visualization Modes: *Force‑Directed* vs *Hierarchical*

These tools allow you to navigate dynamically through the relationship graph between assets.  
They reveal dependencies across layers: upward toward the **abstract / business** level, or downward toward the **concrete / physical** level.

---

## 🕸️ The `Exploration` Tool *(Tools ➡️ Exploration)*

This mode displays a relational graph **without any imposed hierarchy**.  
Nodes are positioned using a physics engine (attraction / repulsion), which makes it possible to:

- freely explore relationships  
- identify clusters  
- navigate complex dependency networks  
- visualize the entire graph around a given asset  

This mode is ideal for **intuitive exploration**.

🔍 *Interface overview available in the page [applications#Mapping Explorer](./application.md/#mapping-explorer)*

---

## ⟁ The `Dependencies` Tool *(Tools ➡️ Dependencies)*

This mode enforces a **layered structure** (upstream → downstream).  
Dependencies are oriented and organized by levels, which makes it possible to:

- quickly understand data flows  
- analyze upstream or downstream dependencies  
- document an architecture  
- clarify complex relationships  

This mode is ideal for **structured analysis**.

🔍 *Interface overview available in the page [applications#Dependency analysis](./application.md/#dependency-analysis)*

---


## 1. Detailed Interface description


| UI Element      | Type          | 🕸️ Explorer |  ⟁ Dependency  | Role                               |
|-----------------|---------------|:--:|:--:|----------------------------------------------------------|
| **Filter**      | Dropdown list | ✅ | ✅ | Filters the cartography views (e.g., `Applications`)     |
| **Attributes**  | Dropdown list | ✅ | ✅ | Filters on the attributes of assets (ex : `Opensource`)  |
...
| **Analyze**     | Green button  |    | ✅ | Launches the Dependency analysis based on the defined parameters |
...
| Filtre | 🕸️ Exploration | ⟁ Dependency |
|--------|:--------------:|:--------------:|
| `Logical Infrastructure` only:<br>RH-Solution does not appear.| [<img src="/mercator/images/filter_exploration_infra.png" width="550">](images/filter_exploration_infra.png) | [<img src="/mercator/images/filter_dependency_infra.png" width="550">](images/filter_dependency_infra.png) |
| `Applications` + `Logical Infrastructure`:<br>RH-Solution and DB-RH-PROD appear. | [<img src="/mercator/images/filter_exploration_full.png" width="550">](images/filter_exploration_full.png) | [<img src="/mercator/images/filter_dependency_full.png" width="550">](images/filter_dependency_full.png) |
...
| Cross-layer exploration or dependency analysis (application + infrastructure)    | Enter **all** relevant views, or leave empty       |
| Full impact analysis (all layers)                         | **Leave the filter empty** to exclude nothing      |
| Exploration or dependency analysis limited to a single layer (e.g. network only) | Enter only that layer's view                       |

> 💡 **Tips:**  
> - When in doubt about what you're looking for, always start with an **empty** filter. You can narrow it down later if the graph becomes too dense.  
> - You can apply a filter, select the object, add it to the window, remove the filter, and then run `🕸️ Deploy` or `⟁ Analyze`.  
> - To filter, you can also type the beginning of the asset names you're searching for in the *Object* field.
...
## 5. Exploration or Dependency Examples
...
- `certificates` (570) are cross-cutting: associate them to both applications AND logical servers for complete
  exploration or Dependency analysis.
