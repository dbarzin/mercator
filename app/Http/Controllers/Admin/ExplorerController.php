...
class ExplorerController extends Controller
{
    public function explore()
    {
        ...
        foreach ($sites as $site) {
            $this->addNode($nodes, 6, $this->formatId('SITE_', $site->id), $site->name, '/images/site.png');
            // link to build
        }
        // BUILDINGS
        $buildings = DB::table('buildings')->select('id', 'name', 'site_id')->whereNull('deleted_at')->get();
        foreach ($buildings as $building) {
            $this->addNode($nodes, 6, $this->formatId('BUILDING_', $building->id), $building->name, '/images/building.png');
        ...
        foreach ($bays as $bay) {
            $this->addNode($nodes, 6, $this->formatId('BAY_', $bay->id), $bay->name, '/images/bay.png');
        ...
        foreach ($physicalServers as $physicalServer) {
            $this->addNode($nodes, 6, $this->formatId('PSERVER_', $physicalServer->id), $physicalServer->name, '/images/server.png');
        ...
        foreach ($workstations as $workstation) {
            $this->addNode($nodes, 6, $this->formatId('WORK_', $workstation->id), $workstation->name, '/images/workstation.png');
        ...
        foreach ($switches as $switch) {
            $this->addNode($nodes, 6, $this->formatId('SWITCH_', $switch->id), $switch->name, '/images/switch.png');
        ...
        foreach ($networks as $network) {
            $this->addNode($nodes, 5, $this->formatId('NETWORK_', $network->id), $network->name, '/images/cloud.png');
        }

        // vlans
        $vlans = DB::table('vlans')->select('id', 'name')->whereNull('deleted_at')->get();
        foreach ($vlans as $vlan) {
            $this->addNode($nodes, 5, $this->formatId('VLAN_', $vlan->id), $vlan->name, '/images/vlan.png');
        }
        ...
        foreach ($subnetworks as $subnetwork) {
            $this->addNode($nodes, 5, $this->formatId('SUBNETWORK_', $subnetwork->id), $subnetwork->name, '/images/network.png', $subnetwork->address);
        ...
        foreach ($logicalServers as $logicalServer) {
            $this->addNode($nodes, 5, $this->formatId('LSERVER_', $logicalServer->id), $logicalServer->name, '/images/lserver.png');
        ...
        foreach ($certificates as $certificate) {
            $this->addNode($nodes, 5, $this->formatId('CERT_', $certificate->id), $certificate->name, '/images/certificate.png');
        }
        ...
        foreach ($zoneAdmins as $zone) {
            $this->addNode($nodes, 4, $this->formatId('ZONE_', $zone->id), $zone->name, '/images/zoneadmin.png');
        }
        // Annuaires
        $annuaires = DB::table('annuaires')->select('id', 'name', 'zone_admin_id')->whereNull('deleted_at')->get();
        foreach ($annuaires as $annuaire) {
            $this->addNode($nodes, 4, $this->formatId('ANNUAIRE_', $annuaire->id), $annuaire->name, '/images/annuaire.png');
        ...
        foreach ($forests as $forest) {
            $this->addNode($nodes, 4, $this->formatId('FOREST_', $forest->id), $forest->name, '/images/ldap.png');
        ...
        foreach ($domains as $domain) {
            $this->addNode($nodes, 4, $this->formatId('DOMAIN_', $domain->id), $domain->name, '/images/domain.png');
        }
        ...
        foreach ($applicationBlocks as $applicationBlock) {
            $this->addNode($nodes, 3, $this->formatId('BLOCK_', $applicationBlock->id), $applicationBlock->name, '/images/applicationblock.png');
        }
        // Applications
        $applications = DB::table('m_applications')->select('id', 'name', 'application_block_id')->whereNull('deleted_at')->get();
        foreach ($applications as $application) {
            $this->addNode($nodes, 3, $this->formatId('APP_', $application->id), $application->name, '/images/application.png');
        ...
        foreach ($services as $service) {
            $this->addNode($nodes, 3, $this->formatId('SERV_', $service->id), $service->name, '/images/service.png');
        }
        ...
        foreach ($modules as $module) {
            $this->addNode($nodes, 3, $this->formatId('MOD_', $module->id), $module->name, '/images/applicationmodule.png');
        }
        ...
        foreach ($databases as $database) {
            $this->addNode($nodes, 3, $this->formatId('DATABASE_', $database->id), $database->name, '/images/database.png');
        }
        ...
        foreach ($informations as $information) {
            $this->addNode($nodes, 2, $this->formatId('INFO_', $information->id), $information->name, '/images/information.png');
        }
        ...
        foreach ($processes as $process) {
            $this->addNode($nodes, 2, $this->formatId('PROCESS_', $process->id), $process->identifiant, '/images/process.png');
        ...
        foreach ($macro_processuses as $macro_process) {
            $this->addNode($nodes, 2, $this->formatId('MACROPROCESS_', $macro_process->id), $macro_process->name, '/images/macroprocess.png');
        }

        // Activities
        $activities = DB::table('activities')->select('id', 'name')->whereNull('deleted_at')->get();
        foreach ($activities as $activity) {
            $this->addNode($nodes, 2, $this->formatId('ACTIVITY_', $activity->id), $activity->name, '/images/activity.png');
        }
        // activity_process
        ...
        foreach ($operations as $operation) {
            $this->addNode($nodes, 2, $this->formatId('OPERATION_', $operation->id), $operation->name, '/images/operation.png');
        }
        ...
        foreach ($actors as $actor) {
            $this->addNode($nodes, 2, $this->formatId('ACTOR_', $actor->id), $actor->name, '/images/actor.png');
        }
        ...
        foreach ($entities as $entity) {
            $this->addNode($nodes, 1, $this->formatId('ENTITY_', $entity->id), $entity->name, '/images/entity.png');
        }

        // Relations
        $relations = DB::table('relations')->select('id', 'name', 'source_id', 'destination_id')->whereNull('deleted_at')->get();
        foreach ($relations as $relation) {
            $this->addNode($nodes, 1, $this->formatId('REL_', $relation->id), $relation->name, '/images/relation.png');
    ...
    private function addNode(&$nodes, $vue, $id, $label, $image, $title = null)
    {
        $data = [ 'vue' => $vue, 'id' => $id,  'label' => $label, 'image' => $image ];
        if($title !== null) {
            $data['title'] = $title;
        }
