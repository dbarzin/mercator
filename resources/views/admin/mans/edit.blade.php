                    ...
                    <div class="col-8">
                        <div class="form-group">
                            <label class="label-maturity-1"
                                   for="description">{{ trans('cruds.man.fields.description') }}</label>
                            <textarea
                                    class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}"
                                    name="description"
                                    id="description">{!! old('description', $man->description) !!}</textarea>
                            @if($errors->has('description'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('description') }}
                                </div>
                            @endif
                            <span class="help-block">{{ trans('cruds.man.fields.description_helper') }}</span>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('wans') ? 'is-invalid' : '' }}"
                                ...
                                @foreach($wans as $id => $name)
                                    <option value="{{ $id }}" {{ (in_array($id, old('wans', [])) || $man->wans->contains($id)) ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('wans'))
                                <div class="invalid-feedback">
                    ...
                    <div class="col-4">
                        <div class="form-group">
                            ...
                            @foreach($mans as $id => $name)
                                <option value="{{ $id }}" {{ ($man->parent_man_id ?? old('parent_man_id')) == $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                            </select>
                        </div>
                    </div>
