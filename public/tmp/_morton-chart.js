...
var require$$0 = ( index && index['default'] ) || index;
...
var hilbert$$1 = function() {
    ...
    var elem,
        ...
        segmentColorScale = ordinal(category20),
        ...
        axisLeft$$1, axisRight$$1, axisTop$$1, axisBottom$$1,
        zoomBox;
    ...
    function init() {
        ...
        if (valTooltip.empty()) {
            valTooltip = select('body').append('div')
                .attr('id', 'val-tooltip');
        }
        ...
        hilbertCanvas.on('mousemove', function () {
            ...
            valTooltip.text(valFormatter(hilbert$$1.getValAtXY(coords[0], coords[1])))
        ...
        axisLeft$$1 = axisLeft(axisScaleY).tickFormat(getTickFormatter(0));
        axisRight$$1 = axisRight(axisScaleY).tickFormat(getTickFormatter(1));
        axisTop$$1 = axisTop(axisScaleX).tickFormat(getTickFormatter(null, 0));
        axisBottom$$1 = axisBottom(axisScaleX).tickFormat(getTickFormatter(null, 1));
        ...
        function getTickFormatter(xZoomBoxIdx, yZoomBoxIdx) {
            return function(d) {
                ...
                return valFormatter(hilbert$$1.getValAtXY(xy[0], xy[1]));
            }
        }
    }
    ...
    function d3Digest(ranges) {
        ...
        var names = ranges.map(function(range$$1) { return range$$1.name; });
        names = names.filter(function(name, idx) {
            return names.indexOf(name) == idx;
        });
        segmentColorScale.domain(names);

        ranges.forEach(hilbert$$1.layout);
        ranges.forEach(function(range$$1) {
            range$$1.startCell = range$$1.pathVertices[0];
        });
        ...
        newPaths.append('path')
            .on('mouseenter', function() { select(this).transition().duration(200).style('opacity', 1); })
            .on('mouseleave', function() { select(this).transition().duration(400).style('opacity', 0.8); });
        ...
        rangePaths.selectAll('text')
            .attr('font-size', function(d) { return min([
    ...
    function getHilbertPath(vertices) {
        var path$$1 = 'M' + vertices[0].join(' ');

        vertices.forEach(function(vert) {
            path$$1 += 'L' + vert.join(',');
        });

        console.log(vertices, path$$1);

        return path$$1;

        vertices.forEach(function(vert) {
            switch(vert) {
                case 'U': path$$1 += 'v-1'; break;
                case 'D': path$$1 += 'v1'; break;
                case 'L': path$$1 += 'h-1'; break;
                case 'R': path$$1 += 'h1'; break;
            }
        });
        return path$$1;
    }

    function applyZoom(zoomTransform$$1) {
        // Translate canvas
        svg.select('.hilbert-canvas').attr('transform', zoomTransform$$1);

        // Adjust axises
        var xScale = zoomTransform$$1.rescaleX(axisScaleX);
        var yScale = zoomTransform$$1.rescaleY(axisScaleY);
        zoomBox[0] = [xScale.domain()[0], yScale.domain()[0]];
        zoomBox[1] = [xScale.domain()[1], yScale.domain()[1]];

        var axises = svg.select('.hilbert-axises');

        axises.select('.axis-left').call(axisLeft$$1.scale(yScale));
        axises.select('.axis-right').call(axisRight$$1.scale(yScale));
        axises.select('.axis-top').call(axisTop$$1.scale(xScale))
            .selectAll('text')
                .attr("x", 9)
                .attr("dy", ".35em")
                .attr("transform", "rotate(-45)")
                .style("text-anchor", "start");
        axises.select('.axis-bottom').call(axisBottom$$1.scale(xScale))
            .selectAll('text')
                .attr("x", -9)
                .attr("dy", ".35em")
                .attr("transform", "rotate(-45)")
    ...
    function focusOn(pos, length) {
        ...
        hilbert$$1.layout(tlPoint);
        tlPoint.startCell = tlPoint.pathVertices[0];

        var zoomTransform$$1 = transform(this);
        zoomTransform$$1.x = -tlPoint.startCell[0] * canvasWidth / length;
        zoomTransform$$1.y = -tlPoint.startCell[1] * canvasWidth / length;
        zoomTransform$$1.k = Math.pow(2, order) / length;

        applyZoom(zoomTransform$$1);
    }
    ...
    function addMarker(pos, markerUrl, width, height, tooltipFormatter) {
        ...
        var marker = svg.select('.markers-canvas').append("svg:image")
            ...
            .attr("width", width)
            .attr("height", height)
            .attr("x", range$$1.startCell[0] * range$$1.cellWidth - width/2)
            .attr("y", range$$1.startCell[1] * range$$1.cellWidth - height/2);

        // Tooltip
        var markerTooltip = index$5()
            .attr('class', 'hilbert-tooltip')
            .offset([-15, 0])
    ...
    function addHeatmapPoints(pnts) {
        var hmData = pnts.map(function(pnt) {
            ...
            hilbert$$1.layout(hPnt);
            hPnt.startCell = hPnt.pathVertices[0];
