...
@section('content')
    ...
    <div class="form-group">
        <a class="btn btn-default" href="{{ route('admin.bpmns.index') }}">
            {{ trans('global.back_to_list') }}
        </a>

        @can('graph_edit')
            <a class="btn btn-info" href="{{ route('admin.bpmns.edit', $graph->id) }}">
                {{ trans('global.edit') }}
            </a>
        @endcan

        @can('graph_create')
            <a class="btn btn-warning" href="{{ route('admin.bpmns.clone', $graph->id) }}">
                {{ trans('global.clone') }}
            </a>
        @endcan

        @can('graph_delete')
            <form action="{{ route('admin.bpmns.destroy', $graph->id) }}" method="POST"
