    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.wifi-terminals.update", [$wifiTerminal->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" type="text" name="type" id="type" value="{{ old('type', $wifiTerminal->type) }}">
                @if($errors->has('type'))
                    <div class="invalid-feedback">
                        {{ $errors->first('type') }}
                    </div>
            ...
            <div class="form-group">
                <label for="bays">{{ trans('cruds.wifiTerminal.fields.bay') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('bays') ? 'is-invalid' : '' }}" name="bays[]" id="bays" multiple>
                    @foreach($bays as $id => $bay)
                        <option value="{{ $id }}" {{ (in_array($id, old('bays', [])) || $wifiTerminal->bays->contains($id)) ? 'selected' : '' }}>{{ $bay }}</option>
                    @endforeach
                </select>
                @if($errors->has('bays'))
                    <div class="invalid-feedback">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.wifiTerminal.fields.bay_helper') }}</span>
            </div>
            <div class="form-group">
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '/admin/wifi-terminals/ckmedia', true);
                ...
                data.append('crud_id', {{ $wifiTerminal->id ?? 0 }});
                xhr.send(data);
              });
            })
        }
