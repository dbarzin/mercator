...
## 🌐 Deux modes de visualisation : *Force‑Directed* vs *Hiérarchique*
Ces outils, permettent de naviguer dynamiquement dans le graphe de relations entre les
assets. Il révèle les dépendances entre couches : vers le **haut** (abstrait / métier) ou vers le **bas** (concret /
physique).

## 🕸️ L'outil `Exploration` *(Outils ➡️ Exploration)*

Ce mode affiche un graphe relationnel **sans hiérarchie imposée**.  
...
🔍 *Présentation de l'interface dans la page [applications#Exploration de la cartographie](./application.fr.md/#exploration-de-la-cartographie)*

## ⟁ L'Outil `Dépendances` *(Outils ➡️ Dépendances)*

Ce mode impose une structure **en couches** (amont → aval).  
...
🔍 *présentation de l'interface dans la page [applications#Analyse des dépendances](./application.fr.md/#analyse-des-dependances)*

---

## 1. Description détaillée des interfaces 
...
> ⚠️ **Point critique** : le champ Filtre a un double effet qu'il est essentiel de comprendre avant d'utilise ces outils.
...
Objet" n'affiche plus que les assets appartenant à cette vue. Cela évite de chercher un asset dans l'intégralité de la CMDB.
...
Concrètement : si vous sélectionnez un `logical-server` avec uniquement `Infrastructure logique` dans le filtre, les
...
| Filtre | 🕸️ Exploration | ⟁ Dependances |
|--------|:--------------:|:--------------:|
...
> 💡 **Conseils** :  
> - en cas de doute sur ce que l'on cherche, commencez toujours avec le filtre **vide**. Vous pouvez le restreindre ensuite si le graphe devient trop dense.  
> - Vous pouvez filtrer, sélectionner l'objet, l'ajouter dans la fenêtre, supprimer le filtre et lancer `🕸️ Déployer` ou `⟁ Analyser`
> - Pour filtrer, Vous pouvez aussi saisir le début des noms des assets recherchés dans le champs objets  
...
## 5. Exemples d'Exploration ou d'analyse
...
### 7.1 Recommandations pour ces outils
...
visibles dans le graphe. Pour une exploration ou analyse cross-couches, pensez à ajouter toutes les vues pertinentes ou à laisser
le filtre vide.

**🕸️ Mode Physique**  
Activer le toggle Physique uniquement quand vous souhaitez visualiser les liaisons réseau physiques (WAN/LAN/MAN).
Désactivé, l'exploration ou l'analyse reste au niveau logique.
